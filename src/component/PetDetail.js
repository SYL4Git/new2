	...
	const handleCalendar = (value) => {
		...
		setIsSymptom(
			pet_symptoms &&
				pet_symptoms.filter(
					(item) =>
						morning <= new Date(item.symptom_date) &&
						new Date(item.symptom_date) <= afternoon
				)
		);
	};
	...
	return (
		<div className="PetDetail">
			...
			{isSymptom && isSymptom.length > 0 ? (
					...
					{isSymptom.map((item) => (
						<div key={item.symptom_date}>
							<div className="symptomList">
								<p>
									<span>기침</span>
									<span>{item.cough}</span>
								</p>
								<p>
									<span>콧물</span>
									<span>{item.runningNose}</span>
								</p>
								<p>
									<span>식욕</span>
								...
								<p>
									<span>소변</span>
									<span>{item.urine}</span>
								</p>
								<p>
									<span>대변</span>
									<span>{item.excrement}</span>
								</p>
								<p>
									<span>체온</span>
				...
				<PetRecord calValue={value} pet_id={pet_id} />
			)}
		</div>
	);
};
