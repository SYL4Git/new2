...
const PetRecord = ({ pet_id, calVlaue }) => {
  ...
  if (!selectedPet) {
    ...
    /* 시나리오: 
  증상기록을 저장하면 증상(key)에 해당하는 정보(value)가 petDummy에 생성


  선택된 pet_id의 calValue와 동일한 pet_id의 calValue에 해당하는 배열 찾기
  찾은 배열 안에는 그에 해당하는 PetRecord의 기록이 존재
  const selectedRecord=petDummy.find((itm)=>pet.pet_id.calValue === parseInt(pet_id.calValue))

  없으면 없다고 경고하기
  if(!selectedRecord){return <p>해당 증상기록을 찾을 수 없습니다.</p>} */
