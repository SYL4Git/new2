...
import { hospitalDummy } from "../public/dummy";
...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "RESVERREMOVE": {
      ...
      newState = state.map((item) => {
        if (item.reservations) {
          // Remove the reservation with the specified reservationId
    ...
    case "SYMPTOMADD": {
      const { symptomData } = action;
      newState = state.map((item) => {
        if (item.pet_id === symptomData.pet_id) {
          return {
            ...item,
            pet_symptoms: [...item.pet_symptoms, symptomData],
          };
        }
        return item;
      });
    ...
    // case 'SYMPTOMEDIT': {
    //     const {symptomID}=action
    //     newState= state.map((item)=> {
    //         if(item.pet_id=== symptomID.pet_id) {
    //             return{
    //                 ...item, pet_symptoms: [...item.pet_sympotms, symptomID]
    //             }
    //         }
    //         return item
    //     })
  ...
  const onSymptomAdd = (
    ...
    dispatch({
      ...
      data: {
        ...
        symptom_date: new Date(date).getTime(),
        cough,
        runningNose,
        hunger,
        urine,
