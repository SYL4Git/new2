...
const Reservation = () => {
  ...
  useEffect(() => {
    if (hospitalList.length >= 1) {
      const targetHos = hospitalList.find(
        (it) => parseInt(it.hos_id) === parseInt(hos_id)
      );
      if (targetHos) {
  ...
  const reserOpen = parseInt(
    reserHos.open_hours && reserHos.open_hours.open_time.substring(0, 2)
  );
  const reserClose = parseInt(
    reserHos.open_hours && reserHos.open_hours.close_time.substring(0, 2)
  );
  ...
  return (
    <div className="Reservation">
      <div className="ReserWrap">
          ...
          {animalList.map((animal) => (
            <div key={animal.pet_id} {...animal}>
              <Button
                btnText={
                  <div>
                    <p className="ani_photo">
                      <img src={animal.pet_photo} />
                    </p>
                    <p>{animal.pet_name}</p>
                  </div>
                }
        ...
        <div className="ReserHospital">{reserHos.hos_name}</div>
